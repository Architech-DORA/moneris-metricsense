...
export const Step1 = ({ from }: Props) => {
  ...
  const selectedConnectionsByunique: { [key: string]: MixConnection } = useMemo(
    () => props.connections.reduce((acc, sc) => ({ ...acc, [sc.unique]: sc }), {}),
    [props.connections],
  );
  const error = useMemo(() => {
    switch (true) {
              ...
              onChangeItems={(selectedItems) => {
                ...
                onChangeConnections(
                  selectedItems.map((sc) => {
                    ...
                    const connection = selectedConnectionsByunique[sc.unique];
                    return {
                      unique: sc.unique,
                      plugin: sc.plugin,
                      connectionId: sc.id,
