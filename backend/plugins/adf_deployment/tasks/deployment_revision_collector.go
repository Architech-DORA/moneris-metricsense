...
const RAW_ADF_DEPLOYMENT_REVISION_TABLE = "adf_deployment_revisions"

var _ plugin.SubTaskEntryPoint = CollectAdfDeploymentRevision

type AdfDeploymentAPIResult struct {
	Data []json.RawMessage `json:"data"`
}

// CollectAdfDeploymentRevision collect all DeploymentRevision that bot is in
func CollectAdfDeploymentRevision(taskCtx plugin.SubTaskContext) errors.Error {
	...
	table := fmt.Sprintf("_raw_%s", RAW_ADF_DEPLOYMENT_REVISION_TABLE)
	...
	for i, rs := range rolloutHistory.Items {
		...
		cicdScopeId := didgen.NewDomainIdGenerator(&models.AdfDeployment{}).Generate(data.Options.ConnectionId, data.Options.DeploymentName)
		...
		msg := map[string]interface{}{
			...
			"cicd_scope_id":      didgen.NewDomainIdGenerator(&models.AdfDeployment{}).Generate(data.Options.ConnectionId, data.Options.DeploymentName),
		}
	...
	logger.Info("Saving data into : %v table", RAW_ADF_DEPLOYMENT_REVISION_TABLE)
...
var CollectAdfDeploymentRevisionsMeta = plugin.SubTaskMeta{
	Name:             "collectAdfDeploymentRevisions",
	EntryPoint:       CollectAdfDeploymentRevision,
	EnabledByDefault: true,
	Description:      "Collect AdfDeploymentRevisions from api",
}
